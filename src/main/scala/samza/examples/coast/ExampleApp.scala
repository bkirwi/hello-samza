...
import com.monovore.coast.samza.{SamzaConfig, ConfigGenerator}
 ...
 */
abstract class ExampleApp(builder: Config => ConfigGenerator) extends Logging {
  ...
  def main(args: Array[String]): Unit = {
    ...
    args.toList match {
      ...
      case "help" :: Nil => printOptions()
      case Nil => println("\nNo arguments!"); printOptions()
      case unknown => println("\nUnrecognized arguments: " + unknown.mkString(" ")); printOptions()
    }
  }

  private[this] def printOptions(): Unit = {
  ...
  private[this] def withBaseConfig(basePath: String): Map[String, Config] = {
    ...
    builder(baseConfig).configure(graph)
  }
